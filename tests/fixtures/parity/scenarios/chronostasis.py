"""Parity scenario: leaves Pattern + callable vel using np.random.uniform.

Exercises `set_pfields(list(leaf_nodes), freq=Pattern([...]), vel=lambda c: ...)`
with a context-taking callable that draws from seeded numpy.random.
"""
...
def build():
    ...
    uc1.set_instrument(list(uc1._rt.leaf_nodes), inst)
    uc1.set_pfields(
        list(uc1._rt.leaf_nodes),
    ...
    uc2.set_instrument(list(uc2._rt.leaf_nodes), inst)
    uc2.set_pfields(
        list(uc2._rt.leaf_nodes),
