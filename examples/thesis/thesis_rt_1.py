...
for j, ut in enumerate(utseq):
    ...
    for i, event in enumerate(ut):
        ...
        scheduler.add_event('random', event['start'], duration=duration, seed=np.random.randint(1000))
        # scheduler.add_event('bassDrum', event['start'])
...
#     scheduler.add_event(synth, event['start'], duration=event['duration']*dur_scale, freq=freq, amp=amp*amp_scale)
