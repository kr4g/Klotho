...
def metric_modulation_ex(metric_ratios: list, pulses: list, bpm: float = 60):
    '''
    '''
    duration_pulse_pairs = topos.ratio_pulse_pairs(metric_ratios, pulses, bpm)
    start_time = 0.0
    rows_list = []
...
# def metric_modulation_test(metric_ratios: list, pulses: list, bpm: float = 60):
#     '''
#     '''
#     ratio_pairs_pulse_ccs = topos.cyclic_cartesian_pairs(metric_ratios, pulses)
#     start_time = 0.0
#     rows_list = []
#     freq_ratios = [tonos.cents_to_ratio(cent) for cent in [600, 0, -700, -100]]
#     # octaves = [0.5, 1.0, 2.0, 3.0]
...
#                                        aikous.Dynamics.mf, aikous.Dynamics.f]) * 0.17
#         root_freq  = (666 * 1.6**-3) * tonos.cents_to_ratio(1067)**octaves[i % 4]
#         attackTime = max([0.005, duration * 0.1])
#         pan        = np.random.uniform(-1.0, 1.0)
#         amFunc     = np.random.randint(0, 3)
#         reverb_min = np.interp(i, [0, len(duration_pulse_pairs)], [0.0, 0.09])
#         reverb_max = np.interp(i, [0, len(duration_pulse_pairs)], [reverb_min, 0.27])
...
if __name__ == '__main__':
    ...
    filename = 'metric_modulation_ex_00.synthSequence'
