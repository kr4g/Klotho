...
The `AlloPy` Python package is composed of five modules ruled by the five dieties of music composition:  *Chronos, Tonos, Topos, Aikous,* and *Skora*.

Each of the five modules contains a base module named after itself as well as various other submodules that specialize in some aspect of their domain (e.g., `Tonos` contains a base module `tonos.py` with basic pitch/frequency tools as well as other submodules dedicated to specific pitch- and frequency-based operations such as `scales.py`).
