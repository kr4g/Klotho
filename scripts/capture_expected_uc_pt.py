...
_sf_spec = _ilu.spec_from_file_location(
    "synth_fixtures",
    _Path(__file__).resolve().parent.parent / "tests" / "synth_fixtures.py",
)
_sf = _ilu.module_from_spec(_sf_spec)
_sf_spec.loader.exec_module(_sf)
ToneInst = _sf.SynthFixtures
...
if __name__ == "__main__":
    ...
    _require('tests/expected_uc_pt.json', must_import_remote=True)
    # -------------------------------------------------------------------
    main()
