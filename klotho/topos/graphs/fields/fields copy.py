...
class Field(ABC):
    def __init__(self, 
        ...
        for point, attrs in space_points.items():
            self.nodes[point] = attrs['field_value']
    ...
    def get_field_value(self, point: Any) -> Any:
        ...
        point = tuple(point) if isinstance(point, np.ndarray) else point
        
        if point in self.nodes:
            return self.nodes[point]
        else:
            # If the point is not in our pre-computed nodes, use the field_function
            return self.field_function(np.array([point])).item()
    ...
    def __setitem__(self, point: Any, value: Any):
        ...
        self.nodes[tuple(point) if isinstance(point, np.ndarray) else point] = value
